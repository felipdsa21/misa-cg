...
def draw_telhado():
    y_base = constantes.asa_size.y + 0.05
    beiral = 0.25
    ridge_alt = 0.9
    z_front = constantes.asa_z_offset - beiral
    z_back = constantes.asa_z_offset + constantes.asa_size.z + beiral
    ...
    c_back = constantes.parte_central_size.z + centro_beiral
    ...
    friso_alt = 0.30
    friso_y = constantes.janela_cima_y_offset + constantes.janela_com_arco.y + 0.25  # base do friso
    if friso_y > constantes.parte_central_size.y - 0.4:
        friso_y = constantes.parte_central_size.y - 0.4  # não invadir telhado central
    primitives.draw_box(
        util.Vec3d(constantes.asa_size.x, friso_y, 0.02),
        util.Vec3d(constantes.parte_central_size.x, friso_alt, 0.18),
    )

    # Frontão curvo mais baixo (mantido)
    frontao_larg = constantes.parte_central_size.x * 0.65
    frontao_alt = 1.8
    frontao_esp = 0.16
    frontao_base_x = constantes.asa_size.x + (constantes.parte_central_size.x - frontao_larg) / 2.0
