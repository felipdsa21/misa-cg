...
Model loadModel(char *fileName) {
  ...
  // total of faces
  objModel.totF = i;
  objModel.f = malloc(i * sizeof(faces));
  rewind(pFile);
  i = 0;
  ...
  while (fgets(mystring, 100, pFile) != NULL) {
    if (strcmp(strtok(mystring, " "), "v") == 0) {
      pch = strtok(NULL, " ");
      if (pch != NULL) {
        objModel.v[i].X = atof(pch);
      }
      pch = strtok(NULL, " ");
      if (pch != NULL) {
        objModel.v[i].Y = atof(pch);
      }
      pch = strtok(NULL, " ");
      if (pch != NULL) {
        objModel.v[i].Z = atof(pch);
      }
      i++;
    }
  }
  ...
  while (fgets(mystring, 100, pFile) != NULL) {
    if (strcmp(strtok(mystring, " "), "f") == 0) {
      if (objModel.f[j].vNumber == 3) {
        pch = strtok(NULL, " ");
        if (pch != NULL) {
          objModel.f[j].Faces[0] = atoi(pch);
        }
        pch = strtok(NULL, " ");
        if (pch != NULL) {
          objModel.f[j].Faces[1] = atoi(pch);
        }
        pch = strtok(NULL, " ");
        if (pch != NULL) {
          objModel.f[j].Faces[2] = atoi(pch);
        }
      }
